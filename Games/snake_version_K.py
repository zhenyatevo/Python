...
# Spesial K
special_font1 = pygame.font.SysFont("comicsansms", 50)
...
def specialK(event_flag):
    while event_flag == 1:
        dis.fill(color_greenyellow)
        mesg1 = special_font1.render('АРМЕНИЯ?!', True, color_for_bg)
        mesg2 = special_font2.render('теперь он пьяный по твоей вине, ЦАРИЦА...', True, color_for_bg)
    ...
    while not game_over:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.QUIT:
                game_over = True
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_k: # Ачивка
                    event_flag *= -1
                    specialK(event_flag)
